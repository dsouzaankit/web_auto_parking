...
- After Zone Apple Pay, `/ondemand-guest-purchase` stores the `/sessions/{uuid}` receipt link under **Z. History → Receipts** (last 7; oldest drops). Rows are keyed by **uuid**, not Zone # — a new pay is a new row even if an older mislabeled ticket (`95347922`) is still in the list. The row’s Zone # is the public 4–6 digit signage (`47922`). Later taps / cart XHR must not replace it with a vendor+zone id (`95347922` was `953`+`47922` after Attempted opened the wrong city). If that capture misses, the timer page (`/sessions/{uuid}`) swaps the wand for **Copy link**; **Paste link** on Z. History saves it. Pasting the same uuid again is a no-op (keeps the original row). Garage **reservations** are not saved there — those already send a confirmation-email pass. Automation does **not** keep running on `/zone/confirmation` or `/sessions/…`.
