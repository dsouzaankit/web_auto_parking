...
- Last **3** attempted Zone checkouts (`internalZoneCode` + signage when known) are stored from `/zone/start?internalZoneCode=` and zone XHR, including across app reboots. They appear under **Z. History → Attempted**; tap one to reopen Confirm Zone. Same Zone # moves to the top; a 4th unique zone drops the oldest. Zone tab still opens `/search` (geo). Confirm Zone stays manual.
...
3. Run **`.\deploy.ps1`** — injects `BookingConfig.json`, strips broken `_CodeSignature`, copies timestamped `WebAutoParking-b{build}-{timestamp}.ipa` to iCloud Downloads (removes older copies). Also starts **AltServer** (tray) via `env_setup\altserver_refresh` (`-SkipAltStorePrep` to skip). Phone-subnet check stays on **USB plug-in**.
