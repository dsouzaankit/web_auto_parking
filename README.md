...
- **Garage Payment Details stall (build 74):** Vehicle **Save & Continue** is forced ~5s after the plate matches even if country/state React match fails. Payment looks for **Continue with Apple Pay** (including disabled buttons) and does not treat the step as done while **Payment Details** is still on screen. Native **Complete Purchase** is never tapped.
