...
| **Zone** | Opens [`app.parkmobile.io/search`](https://app.parkmobile.io/search). Segmented **Duration** (**40m / 1h20 / 2h** and **2h40**, persisted; default **2h40**). **Search Zones** → **Get user location** → nearest zone within ~2.5 km of GPS opens **zone-details** (won’t grab default Atlanta map zones). Automation **pauses** there so you can verify the Zone # (urban GPS), then tap **Park here** yourself; checkout continues after that. If nothing nearby, it pauses on `/search` so you can type a street address; after `/search/{place}` it auto-picks and **does not** require that zone-details confirm. Then **guest-registration** → **add-vehicle** → **duration** → **confirm**, email / plate / duration ≤ cap, Apple Pay prep. |
...
- Prefill caches only on-street `/api/zones/search` (ignores `/search/transient` garage `location_id`s). Auto open of nearest zone needs a real `internalZoneCode` (e.g. `30447328`) **and** distance ≤ ~2.5 km from GPS — never the undated default-map (Atlanta) list. **Geo pick:** opens `/v2/parking/zone-details?areaNo=` then **pauses** (`awaitManualZoneConfirm`) so you verify the public Zone # before tapping **Park here**; after you tap, guest → vehicle → duration → confirm runs. **Address pick** (`/search/{place}`): no zone-details pause — auto **Park here** after pricing settle. **Attempted** row jumps also auto-continue (you already chose that zone). If geo finds nothing nearby, logs `awaitAddressSearch` and stops on `/search` so you can type an address; `/search/{place}` clears the cache and resumes (ranks by API distance to map center) for remote price checks.
