...
|------|------------|----------|-----------|
| Start booking | Open checkout with session window (Z-stamped times; skips Reserve race) | Facility purchase URL opens checkout | Hourly checkout (**per day: 5:30…11:00 → 11:30**, today then +3 days) |
...
- **ParkChirp Harbor lock:** [1525 Harbor Blvd](https://parkchirp.com/facilities/1525-harbor-blvd/) (`1525-harbor-blvd`, facility `96657`) ignores Session ASAP/−15m/−30m and 3–6h duration. Opens **today 5:30 PM → 11:30 PM**. After sign-in + plate, prefill walks candidates until the SPA keeps one as-is: for **each day** (today + up to 3 future), try start **5:30 / 6:00 / … / 11:00 PM** with end **11:30 PM** (`setTimes` / `parkChirpTimesPending`, ~2s settle). Skips dates/times missing from the picker (full/past). Stops at **Checkout**. Login: `autofillHint` for saved **parkchirp.com** credentials. Payment HTTP 500 in PC testing — treat purchase as manual.
- **ParkChirp Harbor SPA rewrite (PC web):** within ~1s they may rewrite deep links when start is past/unavailable or **today is sold out**. Past/invalid starts often become overnight (**next day 12:00 AM → 11:30 PM**, `$30`). A kept evening window ending 11:30 is typically **`$5`**. Walk later start slots on the same day, then later dates — do not thrash unavailable today.
...
| **ParkChirp** | [1525 Harbor Blvd](https://parkchirp.com/facilities/1525-harbor-blvd/?checkout=true&type=hourly) | `/facilities/{slug}/?checkout=true&type=hourly&startTime=…&endTime=…` (unix **wall-as-UTC**; **per day 5:30…11:00→11:30, today then +3 days** — list tag **Fixed duration**; **sign-in required**) |
...
|---------|----------------------------|---------------------|------------------|
| **Start** | **Last 15m** (or ASAP / last 30m) | Same | **5:30…11:00 PM** (per day; walk until accepted) |
