...
| Auth | **Continue as a Guest** (not logged in — see note below) | Guest checkout by default | Cognito **Log In**: sets `autocomplete` only — **does not** JS-focus or JS-fill (that blocks/dismisses Keychain in `WKWebView`). Alert: tap **Email Address** → **Passwords** / key icon. Then auto-**Submit** when password is filled (≥8 / autofill). Never ABM SSO / Create Account |
...
- **ParkMobile guest ≠ account session.** Automation uses **Continue as a Guest**, then prefills `BookingConfig` email/phone. That email may already be registered with ParkMobile, but guest checkout is still a separate transaction — for **garages and Zone**. The booking **does not** show in the logged-in app’s Activity / booking history, and you generally **cannot extend** it from the native app. Extend-in-app (and history) need a **logged-in** web checkout (same credentials as the app), not guest. Guest receipts live in the confirmation email (and any guest mobile-web receipt link), not the account history.
- **Guest throwaway email:** ParkMobile often auto-completes Contact with a temp address (`pm-test…@email.com`) and collapses that step to **Edit**. Automation now opens Contact → overwrites `BookingConfig` email/phone → waits for React to commit → only then taps Apple Pay. Otherwise the receipt goes to the throwaway inbox and never arrives. Check LAN logs for `contactDiag` / `contact Edit tapped` / `awaitContact`. Also check spam/junk; Apple Hide My Email addresses often land there.
- **Guest “top-up” (new intersecting session):** **Zone (street)** — for prepaid duration zones, ParkMobile allows starting a **new** session while an earlier one is still active (same plate/zone); overlap is usually fine for compliance if each session is valid, but you still can’t exceed the zone’s **posted max**, and some areas enforce a **no-return** window after max (new session blocked ~30m). Prefer real **Extend** when logged in. **Our garages (checked on their ParkMobile pages):** neither [Bisby `59277`](https://app.parkmobile.io/reservation/59277) nor [Harbor `62713`](https://app.parkmobile.io/reservation/62713) publishes “reservations cannot be combined” copy (unlike some other PM lots). Bisby: LPR — *“parking pass is validated by your license plate.”* Harbor: pull a gate ticket, then on exit tell the remote attendant your **parking pass #**. That still does **not** confirm a second overlapping guest booking extends the first — exit is tied to one pass/window (Harbor especially), so intersecting garage top-up remains unverified / not a safe default.
- **Button labels are brittle.** The table above is the providers’ current copy (e.g. ParkMobile garage/vehicle **Save & Continue** vs bare **Continue**). Prefill prefers stable hooks when present (`data-pmtest-id`, `data-testid`, fields like `#vrn`) and falls back to visible label text. A provider rename or layout change can stall a step even though the form looks filled — check LAN logs (`vehicleDiag`, `paymentDiag`, `Save & Continue tapped` / `forced`, `Continue with Apple Pay tapped` / `forced`, `vehicle Continue tapped btn=…`) and re-anchor selectors in `BookingFormPrefill.swift`.
- **Garage Payment Details stall (build 74+):** Vehicle **Save & Continue** is forced ~5s after the plate matches even if country/state React match fails. Payment is **not** a **Save & Continue** label. Live dump (`ai/logs/html/bodyscript.txt`) asserts `<button type="outline">` inside `[data-pmtest-id="guest-payment-step-card"]` with text **Continue with** + `<img alt="applepay icon">` (no `data-pmtest-id` on the button). Native **Complete Purchase** is never tapped. LAN: `paymentDiag … btn=<BUTTON type=outline> img=applepay icon`.
...
- SpotHero’s vehicle popup has **State or Province** only (no Country field). Do not Confirm until **Make and Model** is a dropdown selection (typed text is not enough). If the row shows **No Vehicle · {plate}**, tap **Change** (`VehicleInfo-button-change-vehicle`) and finish the modal. LAN: `spotHeroVehicle display=… needFill=`.
...
**If the screen looks stuck:** say so (which step). You do not need to describe buttons or HTML. Expected pauses already need you: street-address search (`awaitAddressSearch`), ParkChirp **Passwords**, captcha, and native **Complete Purchase** (never tapped). Anything else sitting still is a stall.
...
| **Zone** | Opens [`app.parkmobile.io/search`](https://app.parkmobile.io/search). Segmented **Duration** (**40m / 1h20 / 2h** and **2h40**, persisted; default **2h40**). **Search Zones** → **Get user location** (or typed `/search/{place}`) → nearest zone opens **zone-details** (geo only if within ~2.5 km; won’t grab default Atlanta map zones). Automation **pauses** there after a search nearest pick so you verify the Zone #, then tap **Park here**; checkout continues after that. **Attempted** jumps auto-continue Park here. If geo finds nothing nearby, it pauses on `/search` so you can type an address. Then **guest-registration** → **add-vehicle** → **duration** → **confirm**, email / plate / duration ≤ cap, Apple Pay prep. |
| **Z. History** | **Attempted** (last **3** Zone checkouts) above **Receipts** (last **7** paid `/sessions/{uuid}` links). Oldest row drops when a section is full. Not garage reservations. Tap an attempt to reopen Confirm Zone; tap a receipt to reopen the timer (no automation). Backup: on the post-checkout timer page tap **Copy link**, then **Paste link** here. Same uuid paste is ignored (no extra row). |

### Zone flow notes

- Allow **Location** when prompted so ParkMobile can prefill the nearest zone.
- Prefill caches only on-street `/api/zones/search` (ignores `/search/transient` garage `location_id`s). Auto open of nearest zone needs a real `internalZoneCode` (e.g. `30447328`) **and** (for geo) distance ≤ ~2.5 km from GPS — never the undated default-map (Atlanta) list. **Geo or address nearest pick:** opens `/v2/parking/zone-details?areaNo=` then **pauses** (`awaitManualZoneConfirm`) so you verify the public Zone # before tapping **Park here** (address search still auto-picks nearest to map center, which can be wrong). The confirm flag is stored in **`sessionStorage`** before navigation (a bare `window.*` flag is wiped on zone-details load). After you tap, guest → vehicle → duration → confirm runs. **Attempted** row jumps use a fresh WebView (no flag) and **auto-continue** Park here after pricing settle. If geo finds nothing nearby, logs `awaitAddressSearch` and stops on `/search` so you can type an address; `/search/{place}` clears the cache and resumes (ranks by API distance to map center).
- Last **3** attempted Zone checkouts are stored from `/zone/start?internalZoneCode=` / `/v2/parking/zone-details?areaNo=` and **single-zone** XHR (`zoneoptions` / `parkmobileapi/zones/{internal}`), including across app reboots. The jump URL uses the **internal** code only (7+ digits, e.g. `30447328` / `30447039` / `1081972`). The public Zone # (`47922`) is the label. Same internal code is one row (retry moves it to the top); a 4th unique internal drops the oldest. Map `/zones/search` lists, `/sessions/` receipts, and pay XHR do **not** write Attempted — ingesting search JSON was landing default-map cities (Austin/Atlanta) under a Hoboken label, and using `47922` as `internalZoneCode` opens the wrong city. Zone tab still opens `/search` (geo).
- **ParkMobile Zone v2 checkout (2026):** after Park Here, the SPA redirects to `/v2/parking/zone-details?areaNo={internal}` (not the old Confirm Zone `/zone/start` form). Flow: **zone-details** → **guest-registration** (`guest-registration-email-input`) → **add-vehicle** (`vehicle-form-license-plate-input` + country/state overlays + **Add vehicle**) → **duration** (20m steps, cap from Zone Duration picker) → **confirm** (Apple Pay prep only). LAN: `v2ParkingDiag step=…`, `v2 Add vehicle tapped`, `v2 duration picked …`.
- **Legacy Zone path (still supported):** if ParkMobile serves the pre-v2 pages, prefill still drives `/zone/start` → Confirm Zone (manual submit) → `/zone/auth|vehicle|duration|payment|review`. **Receipts** still capture `POST /ondemand-guest-purchase` + `/sessions/{uuid}` as well as v2 purchase/session URLs. **Attempted** still records `/zone/start?internalZoneCode=` as well as `zone-details?areaNo=`. New jumps prefer v2 zone-details; garage reservation checkout is unchanged (separate from Zone SPA).
- **Brief “An error occurred” flash (ParkMobile, not ours):** tapping **Park here** on `zone-details` before pricing/session setup finishes can land guest-registration on ParkMobile’s error sheet (**We’re sorry… / Try again**). Prefill waits for first-hour pricing (or ~2.8s) before any **auto** Park here (**Attempted** jumps); search nearest picks wait for your manual tap (`awaitManualZoneConfirm`). Then dismisses the overlay / taps **Try again** and continues. A short flash can still happen if their SPA races; if recovery stalls on `/v2/parking` or `zones/map`, prefill reopens cached `areaNo` zone-details. LAN: `awaitManualZoneConfirm`, `v2 zone-details waiting settle`, `v2 Try again tapped`, `v2 error overlay closed`, `v2 recover zone-details`.
...
- After Zone Apple Pay, paid guest receipts are stored under **Z. History → Receipts** (last 7; oldest drops). Capture sources: legacy `POST /ondemand-guest-purchase` + `/sessions/{uuid}`, and **v2** `POST /v2/parking/api/order/purchase` + `GET /v2/parking/api/parking/{uuid}` + page `/v2/parking/session/guest/{uuid}`. Rows are keyed by **uuid**, not Zone #. The row’s Zone # is the public 4–6 digit signage (`47922`). If that capture misses, the timer/manage page swaps the wand for **Copy link**; **Paste link** on Z. History saves it. Pasting the same uuid again is a no-op (keeps the original row). Garage **reservations** are not saved there — those already send a confirmation-email pass. Automation does **not** keep running on `/zone/confirmation` or `/sessions/…` / v2 session pages.
- **Start a new Zone search after pay:** the receipt/timer can stay on screen. Toolbar **Reload** only refreshes the **current** URL (usually `/sessions/{uuid}`) — it does **not** return to `/search` or re-run Search Zones + geo. Switching **Garages ↔ Zone** does not remount that WebView. On ParkMobile’s page use **⋯ → Find Parking**. Or force-quit the app so the Zone tab loads [`/search`](https://app.parkmobile.io/search) again with the native geo stub. App toolbar is back / forward / reload / wand (or Copy link) / share — not Find Parking.
...
- **Why:** Guest Zone receipts have no confirmation #, barcode, or `orderUuid` (null on on-demand). The public unique key is `parking_uuid` (already the `/sessions/{uuid}` link and Z. History receipt row id) but it is too long for a list. `GET /v2/parking/{uuid}` also returns integer **`id`** (e.g. `110787034` on the 10:02 PM `47922` ticket) — ParkMobile’s parking-row PK, 1:1 with that booking, not an Apple Pay txn. Shorter lookup on the receipt list.
- **Where:** **Z. History → Receipts** only (not garage reservations). Source: `GET https://app.parkmobile.io/api/proxy/parkmobileapi/v2/parking/{uuid}` after pay (timer page). **Not** in `POST /ondemand-guest-purchase` (that JSON is uuid + start/stop only). Example bodies: `ai/parkmobile_zone_xhr/live_xhr.txt` (10:02 PM); `ai/logs/checkout_081526_0909p.txt` is the 8:46 PM run but has no `res:` bodies so no `id` there.
...
5. **Expired ~7-day cert (SideStore):** **no PC** — LocalDevVPN + Wi‑Fi → SideStore → tap **7 DAYS** / Refresh (works after expiry). **Optional:** Shortcuts → **Refresh All Apps** + Personal Automation (daily or 2×/week, **Run Immediately**). Caveats: needs Wi‑Fi + LocalDevVPN; flaky (timeouts / pairing / VPN races); often fails while **locked**; does **not** tap Trust; grant **Always Allow** on first run; still check weekly. Details: [Loop Segments — SideStore Shortcuts auto-refresh](../ios_3d_loop_segments/ios/BUILD-WITHOUT-MAC.md#sidestore-shortcuts-auto-refresh-optional). AltStore still needs AltServer + USB **Refresh All**.
