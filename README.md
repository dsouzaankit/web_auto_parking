...
- Last attempted Zone # (`internalZoneCode` + signage when known) is stored in UserDefaults from `/zone/start?internalZoneCode=` and zone XHR, including across app reboots. Zone tab still opens `/search` (geo). **Zone {n}** on the Duration bar opens that Confirm Zone; **Search** returns to `/search`. Confirm Zone stays manual.
