...
struct BookingConfig: Codable, Equatable {
    ...
    /// Fixed-duration locked window length (1–6). Overridable in-app.
    var sessionDurationHours: Int
    var vehicle: VehicleDetails

    /// Allowed fixed-duration session lengths (Garages tab: 3–6h row, then 1–2h).
    static let allowedDurations = [1, 2, 3, 4, 5, 6]
    ...
    /// Normalized to 1–6 hours (default 4).
