...
<#
.SYNOPSIS
  Copy WebAutoParking.ipa to iCloud Drive Downloads with a unique name (Files refresh).

.DESCRIPTION
  Same split as ios_3d_loop_segments\copy-to-icloud.ps1: stamp the destination IPA
  (WebAutoParking-b{build}-{yyyyMMdd-HHmmss}.ipa), remove older WebAutoParking*.ipa
  from iCloud Downloads, then copy. Injects local BookingConfig.json and strips
  _CodeSignature. Does not trigger GitHub Actions.
...
if (-not (Test-Path -LiteralPath $LocalIpa)) {
    throw "IPA not found: $LocalIpa`nBuild/download it first (Actions -> ios-build -> WebAutoParking-ipa)."
}
...
if (Test-Path -LiteralPath $LocalBookingConfig) {
    ...
    Inject-BookingConfigIntoIpa -IpaPath $LocalIpa -ConfigPath $LocalBookingConfig -OutPath $PreparedIpa
...
if ($leftover.Count -gt 0) {
    ...
    throw "iCloud still has old IPAs. Close Files/AltStore, delete them manually, re-run copy-to-icloud."
}
